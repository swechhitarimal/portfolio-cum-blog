...
const typeColors: Record<string, string> = { book: "text-[var(--color-accent)]", movie: "text-[var(--color-burgundy)]", show: "text-[var(--color-sky)]", podcast: "text-[var(--color-lavender)]", music: "text-[var(--color-amber)]" };
            ...
            {filtered.map((item, i) => {
              ...
              return (
                <ScrollReveal key={item.id} delay={i * 0.05}>
                  <div className="glass rounded-2xl p-5 group hover:border-[var(--color-accent)]/20 transition-all duration-300">
                    <div className="flex items-center gap-2 mb-3">
                      <Icon size={14} className={typeColors[item.type]} />
                      <span className="text-xs px-2 py-0.5 rounded-full border border-[var(--color-border)] text-[var(--color-text-secondary)] capitalize">{item.type}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full ml-auto ${item.status === "finished" ? "bg-[var(--color-accent)]/10 text-[var(--color-accent)]" : item.status === "consuming" ? "bg-[var(--color-amber)]/10 text-[var(--color-amber)]" : "bg-[var(--color-burgundy)]/10 text-[var(--color-burgundy)]"}`}>
                        {item.status}
                      </span>
                    </div>
                    <h3 className="text-base font-semibold mb-1 group-hover:text-[var(--color-accent)] transition-colors">{item.title}</h3>
                    <p className="text-xs text-[var(--color-text-secondary)] mb-3">{item.creator}</p>
                    <p className="text-sm text-[var(--color-text-secondary)] font-light leading-relaxed">"{item.thoughts}"</p>
                    {item.rating && (
                      <div className="flex items-center gap-0.5 mt-3">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star key={i} size={12} className={i < item.rating! ? "text-[var(--color-burgundy)] fill-[var(--color-burgundy)]" : "text-[var(--color-border)]"} />
                        ))}
                      </div>
                    )}
                  </div>
